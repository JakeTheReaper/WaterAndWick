<?php
		...
		$productTree = array (
				'BDC001' => array (
						...
						'pDesc' => '<p>A very groovy scent, Bohemian Dream is inspired by Nag Champa incense. Full-bodied with enticing earthy notes, its sweet warmth is renowned, a fragrance unmistakable and ever so popular.</p><p><strong>Top Notes:</strong> Neroli and fir needle</p><p><strong>Middle Notes:</strong> Coconut husk, lily of the valley and rosewood</p><p><strong>Base Notes:</strong> Musk, woody and rose petals</p>',
						'price' => array (
								'p1' => '25.00',
								'p2' => '50.00'
						),
        ...
        <div class="row">
            <div>
                <h3 class="desc-heading">
                    <?php echo $productTree[PID]['pTitle']; ?>
                </h3>
            </div>
        </div>
        <div class="row">
            ...
            <div class="one-half column">
                ...
                <div class="one-half column price">
                    <h2 id="candle-price">$
                        <?php echo $productTree[PID]['price']['p1']; ?>
                    </h2>
                </div>
                ...
                <form action="cart.php?action=add&PID=" method="post" onsubmit="return inputCheck();">
                    ...
                    <select name="option">
                <option id="small" value="<?php echo $productTree[PID]['options']['BD1']; ?>">Small</option>
                <option id="large" value="<?php echo $productTree[PID]['options']['BD2']; ?>" >Large</option>
            </select>
                    ...
                    <button class="button button-primary qty-add" onclick="incrementQuantity('<?php echo $productTree[PID]['price']['p1']; ?>')" type="button" aria-label="Add 1" title="Add 1" aria-controls="qty"><i class="fa fa-plus-square fa-6" aria-hidden="true"></i></button>

                    <button id="updateTotal" type="button" onclick="totalPrice()">UPDATE TOTAL</button>
                    <input class="button-primary" name="add" type="submit" value="ADD TO CART">
                </form>
            </div>
        </div>
    </div>
